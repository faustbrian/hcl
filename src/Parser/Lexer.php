 ...
 *
 * Performs lexical analysis on HCL source code, converting raw text
 * into structured tokens for parsing. Handles strings, numbers, heredocs,
 * comments, operators, and HCL-specific syntax elements.
 *
...
final class Lexer
{
     ...
     *
     * @throws MalformedHeredocException    If a heredoc literal is malformed
     * @throws UnexpectedCharacterException If an unexpected character is encountered
     * @throws UnterminatedCommentException If a multi-line comment is not properly terminated
    ...
    private function nextToken(): ?Token
    {
        ...
        $token = match ($char) {
            ...
            default => throw UnexpectedCharacterException::at($char, $startLine, $startColumn),
        };
     ...
     * @param  int                       $startLine   The starting line number
     * @param  int                       $startColumn The starting column number
     * @throws MalformedHeredocException If the heredoc is malformed
    ...
    private function readHeredoc(int $startLine, int $startColumn): Token
    {
        ...
        if ($delimiter === '') {
            throw MalformedHeredocException::at($startLine, $startColumn, 'missing delimiter');
        }

        // Expect newline after delimiter
        if ($this->current() !== "\n") {
            throw MalformedHeredocException::at($startLine, $startColumn, 'expected newline after delimiter');
        }
     ...
     * @param  int                         $startLine   The starting line number
     * @param  int                         $startColumn The starting column number
     * @throws UnterminatedStringException If the string is unterminated
    ...
    private function readString(int $startLine, int $startColumn): Token
    {
        ...
        if ($this->isAtEnd()) {
            throw UnterminatedStringException::at($startLine, $startColumn);
        }
     ...
     * @param  int                          $startLine   The starting line number
     * @param  int                          $startColumn The starting column number
     * @throws UnterminatedCommentException If the comment is unterminated
     * @return Token                        The comment token
     */
    private function readMultiLineComment(int $startLine, int $startColumn): Token
    {
        ...
        throw UnterminatedCommentException::at($startLine, $startColumn);
    }

    /**
     * Skip whitespace characters (except newlines).
     *
     * Advances past spaces, tabs, and carriage returns while preserving
     * newlines as significant tokens for HCL parsing.
     */
     ...
     *
     * Increments both position and column tracking.
     */
